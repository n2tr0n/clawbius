...
function get_focused_win() {
    const focused = electron.BrowserWindow.getFocusedWindow();
    if (focused && docs[focused.id]) return docs[focused.id].win;
    const ids = Object.keys(docs);
    for (let i = ids.length - 1; i >= 0; i--) {
        if (!docs[ids[i]].destroyed) return docs[ids[i]].win;
    }
    return null;
}
...
electron.app.on("ready", (event) => {
    ...
    api_server.start(get_focused_win);
});
...
electron.ipcMain.on("api:result", (event, {id, result, error}) => {
    api_server.handle_result({id, result, error});
});
